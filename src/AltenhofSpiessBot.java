...
public class AltenhofSpiessBot extends HolsDerGeierSpieler{
    ...
    /*     
    Festlegen der Grenzwerte f�r die Strategien:
    Hoch-Strategie: spielt Strategie wenn Punktekarte gr��er als grenzSpielKarten[0]
    ...
    Geier-Strategie: spielt Strategie bei allen Geierkarten 
    				 ...
    				 Wenn Geierkarte gr��er gleich grenzPunkteKarten[1] --> spielt Karten aus niederem Bereich
    */
    ...
    Taktik bew�hrt mit folgenden Werten: (7,-2) (12,6) (vs. Fredare); (7,-2) (9,4) (vs. FloRon)  
    */
    ...
    private int hochStrategie() {
        int erg = 0;
        int hKarte = 0; // R�ckgabe Karte

        // Wenn meine h�chste Karte gr��er ist als die des Gegners:
        if (memEigene.get(0)>memGegner.get(0)) {
            ...
            // gehe zur Karte zur�ck die h�her war als die h�chste des Gegners
            if (hKarte > 0) hKarte--;

            fehlerLog("ErrorID: 11");
            erg = memEigene.get(hKarte);

            // Wenn die h�chste Gegnerkarte gleich meiner h�chsten Karte ist
        } else if (memEigene.get(0)== memGegner.get(0)) {
            fehlerLog("ErrorID: 12");
            erg = memEigene.get(0);

            // Wenn Gegner die h�here Karte hat
        } else { // spiele h�chste Karte im MittlerenBereich
    ...
    private int mittelStrategie() {
        ...
        // Wenn meine h�chste Karte des mittleren Sektors gr��er ist als die h�chste Karte des Gegners:
        if (memEigeneMittel.size() > 0 && memEigeneMittel.get(0)>memGegner.get(0)) {
	    	...
	    	/* Wenn die h�chste Karte des mittleren Spielraums gr��er als die h�chste Karte des Gegners ist
	    	   spiele aus dem mittleren Bereich die h�chste Karte */
        } else if (memEigeneMittel.size() > 0 && memEigeneMittel.get(0)== memGegner.get(0)) {
            fehlerLog("ErrorID: 15");
            erg = memEigeneMittel.get(0);
        } else {
            // Wenn Gegner die h�here Karte hat spiele aus dem niederen Bereich die h�chste Karte
    ...
    private int ausnahme(int start) {
        ...
        //1: Niedrig: - Mittel(h�chste) - Alle(h�chste)
        //2: Mittel: - Hoch - Alle
        //3: Hoch: - Mittel - Alle
        //4: Mittel: - Niedrig - Alle
        //5: Niedrig: - Mittel(zuf�llig) - Alle(zuf�llig)
        switch(start) {
            case 1:{
